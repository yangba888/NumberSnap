...
- “自动分列”默认开启，可自动恢复多列；关闭后按单列输出。
- “文数分列”默认开启；关闭 Numbers Only 后，会按整张截图结构输出“名称 + 数量”两列，
  或“序号 + 名称 + 数量”三列，并为缺失字段保留空单元格。
...
- ＜（＾－＾）＞仅识别数字这一项特别适用于工作中报计划。
- ×××功能仅方便识别文字进行复制粘贴，不能贴图/保存图片/复制粘贴图片/编辑图片。
...
商品名称与数量分列时，取消勾选 `Numbers Only` 并保留“文数分列”。例如
`维生素B12 30` 会输出为 `维生素B12<Tab>30`。黄色预览行表示置信度较低或未找到
末尾数量，可直接在预览中修改，再点击“重新复制”。
...
numbersnap/
  ...
  core/
    ...
    text_number_splitter.py # 商品名称与末尾数量分离
...
布局检测先以 Bounding Box 中心 Y 与动态中位高度阈值聚类行。列数由水平空白最清晰的
完整行初始化，再以所有行的左边缘、中心和右边缘中位数迭代校正。稀疏行通过保持从左到右
顺序的动态规划映射到列，所以中间缺失值能输出为空字段。该实现没有固定像素阈值，模块可独立替换。
